...
document.addEventListener('DOMContentLoaded', () => {
        ...
        $('#load-csv').on('click', function() {
            ...
            Papa.parse("daily_ibm.csv", {
                ...
                complete: function(results) {
                    ...
                    // Get CSV columns and create table headers
                    const columns = Object.keys(results.data[0]);
                    columns.forEach(column => {
                        $('#table-head').append(`<th>${column}</th>`);
                    });
    ...
    $(document).ready(function() {
        // Load stock details on button click
        $('#get-details').on('click', function() {
            const symbol = $('#stock-symbol').val().toUpperCase();

            if (symbol) {
                // Example data fetch for stock details (replace with actual API call if available)
                const stockDetails = {
                    "IBM": {
                ...
                if (stockDetails[symbol]) {
                    $('#company-name').text(stockDetails[symbol].name);
                    $('#company-symbol').text(stockDetails[symbol].symbol);
                    $('#current-price').text(stockDetails[symbol].currentPrice);
                    $('#market-cap').text(stockDetails[symbol].marketCap);
                } else {
                    $('#company-name').text("N/A");
                    $('#company-symbol').text("N/A");
